...
class BIP39MnemonicGenerator:
    ...
    def set_entropy(self, entropy_bin):
        if len(entropy_bin) == self.entropy_bits and all(c in '01' for c in entropy_bin):
            self.entropy_bin = entropy_bin
        else:
            raise ValueError(f"Entropy must be a binary string of length {self.entropy_bits}")
    ...
    def mnemonic(self):
        if hasattr(self, 'entropy_bin'):
            entropy_bin = self.entropy_bin
        else:
